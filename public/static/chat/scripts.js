...
function deleteChatHistory(userId) {
    if (!confirm("你他娘的可要想好,真的要删除吗")) {
        return false
    }

    fetch(MODEL +'delMsg?user=' + USER_ID, {
        method: 'DELETE',
...
function getHistory() {
    var formData = new FormData();
    formData.append('user_id', USER_ID);
    fetch(MODEL+'getMsg', {method: 'POST', body: formData})
        .then(response => response.json())
        .then(chatHistory => {
...
function sendMsg(msg) {
    ...
    fetch(MODEL+'sendMsg', {method: 'POST', body: formData})
        .then(response => response.json())
        .then(data => {

            let uuid = uuidv4()
            const eventSource = new EventSource(MODEL+`eventStream?chat_history_id=${data.id}&id=${encodeURIComponent(USER_ID)}`);
